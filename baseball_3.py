...
"""
Refactored to move repeated code into own functions - check_run(), check_move()
"""
...
def check_run(base):
    ...
    if bases[base] is not None:
        ...
        teams[batting].append(bases[base])
        bases[base] = None

def check_move(base, score):
    """If there is a player on the base, move to next base."""
    if bases[base] is not None:
        bases[base+score] = bases[base]
        bases[base] = None
...
def handle_score(score):
    ...
    if score == 1:
        check_run(base=3)
        check_move(base=2, score=1)
        check_move(base=1, score=1)
        check_move(base=0, score=1)
    elif score == 2:
        check_run(base=3)
        check_run(base=2)
        check_move(base=1, score=2)
        check_move(base=0, score=2)
    elif score == 3:
        check_run(base=3)
        check_run(base=2)
        check_run(base=1)
        check_move(base=0, score=3)
    elif score == 4:
        check_run(base=3)
        check_run(base=2)
        check_run(base=1)
